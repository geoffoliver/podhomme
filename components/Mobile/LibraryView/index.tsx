    ...
    return (
      <div className={styles.view}>
        <div className={styles.header}>
          ...
          <span className={styles.headerMeta}>{podcasts.length} podcast{podcasts.length !== 1 ? 's' : ''}</span>
        </div>
        {podcasts.length === 0 ? (
          ...
          <div className={styles.list}>
            {podcasts.map(p => (
              <button key={p.id} className={styles.podcastRow} onClick={() => setSelectedId(p.id)}>
                {p.imageUrl ? (
                  <Image src={p.imageUrl} alt="" width={52} height={52} className={styles.artwork} />
                ) : (
                  <div className={styles.artworkPlaceholder}><Podcast size={22} /></div>
                )}
                <div className={styles.podcastInfo}>
                  <span className={styles.podcastTitle}>{p.title}</span>
                  {p.author && <span className={styles.podcastAuthor}>{p.author}</span>}
                </div>
  ...
  return (
    <div className={styles.view}>
      <div className={styles.podcastHeader}>
        <button className={styles.backBtn} onClick={() => setSelectedId(null)} aria-label="Back to library">
          <ChevronLeft size={22} />
        </button>
        {podcast ? (
          <>
            {podcast.imageUrl ? (
              <Image src={podcast.imageUrl} alt="" width={44} height={44} className={styles.headerArtwork} />
            ) : (
              <div className={styles.headerArtworkPlaceholder}><Podcast size={20} /></div>
            )}
        ...
        {episodes.map(ep => {
          ...
          return (
            <div key={ep.id} className={`${styles.episodeRow} ${ep.played ? styles.episodeRowPlayed : ''}`}>
              ...
              >
                {artUrl ? (
                  <Image src={artUrl} alt="" width={48} height={48} className={styles.episodeArtwork} />
                ) : (
                  <div className={styles.episodeArtworkPlaceholder}><Podcast size={18} /></div>
                )}
