...
function makeFeedEpisode(overrides: Partial<{
  guid: string;
  pubDate: Date;
  title: string;
}> = {}) {
...
describe('refreshPodcast', () => {
  ...
  it('skips episodes that already exist in the database', async () => {
    await db.episode.create({
      data: { podcastId, guid: 'ep-001', title: 'E1', audioUrl: 'u', pubDate: YESTERDAY },
    });
    await refreshPodcast(podcastId);
  ...
  describe('when the feed contains multiple new episodes', () => {
    ...
    it('marks older new episodes as played immediately', async () => {
      await refreshPodcast(podcastId);
      const older = await db.episode.findFirst({ where: { podcastId, guid: 'ep-old' } });
  ...
  describe('60-day rule', () => {
    beforeEach(() => {
      mockParseFeed.mockResolvedValue(makeFeed([
        makeFeedEpisode({ pubDate: SIXTY_ONE_DAYS_AGO }),
      ]));
    });
  ...
  describe('reappearing pruned episode', () => {
    it('creates the episode as played when its pubDate is older than the most recent stored episode', async () => {
      ...
      await db.episode.create({
        data: { podcastId, guid: 'ep-stored', title: 'Stored', audioUrl: 'u', pubDate: YESTERDAY },
      });
      ...
      const pruned = await db.episode.findFirst({ where: { podcastId, guid: 'ep-pruned' } });
  ...
  describe('pruning', () => {
    ...
    it('skips pruning when defaultPlayback is stream (default)', async () => {
      await db.episode.create({
        data: {
          podcastId, guid: 'ep-dl', title: 'Downloaded', audioUrl: 'u',
          pubDate: YESTERDAY, downloadPath: '/downloads/ep.mp3',
        },
      });
      await refreshPodcast(podcastId);
    ...
    describe('numeric episodesToKeep limit', () => {
      beforeEach(async () => {
        ...
        await db.episode.createMany({
          data: [
            { podcastId, guid: 'ep-a', title: 'A', audioUrl: 'u', pubDate: new Date('2024-01-01'), downloadPath: '/dl/a.mp3' },
            { podcastId, guid: 'ep-b', title: 'B', audioUrl: 'u', pubDate: new Date('2024-02-01'), downloadPath: '/dl/b.mp3' },
            { podcastId, guid: 'ep-c', title: 'C', audioUrl: 'u', pubDate: new Date('2024-03-01'), downloadPath: '/dl/c.mp3' },
          ],
        });
      });
      ...
      it('clears downloadPath in the database for pruned episodes', async () => {
        await refreshPodcast(podcastId);
        const epA = await db.episode.findFirst({ where: { podcastId, guid: 'ep-a' } });
        const epB = await db.episode.findFirst({ where: { podcastId, guid: 'ep-b' } });
        const epC = await db.episode.findFirst({ where: { podcastId, guid: 'ep-c' } });
    ...
    describe('episodesToKeep: "all_unplayed"', () => {
      beforeEach(async () => {
        ...
        await db.episode.createMany({
          data: [
            ...
            { podcastId, guid: 'ep-played', title: 'Played', audioUrl: 'u', pubDate: TWO_DAYS_AGO, played: true, downloadPath: '/dl/played.mp3' },
            // unplayed + downloaded → should be kept
            { podcastId, guid: 'ep-unplayed', title: 'Unplayed', audioUrl: 'u', pubDate: YESTERDAY, played: false, downloadPath: '/dl/unplayed.mp3' },
          ],
        });
      });
      ...
      it('preserves download files for unplayed episodes', async () => {
        await refreshPodcast(podcastId);
        const unplayed = await db.episode.findFirst({ where: { podcastId, guid: 'ep-unplayed' } });
...
describe('refreshAll', () => {
  ...
  it('continues refreshing remaining podcasts when one fails', async () => {
    ...
    const bEpisodes = await db.episode.findMany({ where: { podcastId: podB.id } });
  ...
  it('broadcasts per-podcast progress events', async () => {
    ...
    const refreshCalls = mockBroadcast.mock.calls.filter(([event]) => event === 'refresh');
    const progressCalls = refreshCalls.filter(([, data]) => !(data as any).done);
    expect(progressCalls).toHaveLength(2);
    expect(progressCalls[0][1]).toMatchObject({ current: 1, total: 2, done: false });
    expect(progressCalls[1][1]).toMatchObject({ current: 2, total: 2, done: false });
  });
