...
describe('GET /api/export/opml', () => {
  ...
  it('includes an outline element for each subscribed podcast', async () => {
    await db.podcast.create({
      data: { title: 'Pod A', feedUrl: 'https://a.example.com/feed.rss' },
    });
    await db.podcast.create({
      data: {
        title: 'Pod B',
  ...
  it('includes the podcast title in the outline', async () => {
    await db.podcast.create({
      data: {
        ...
        feedUrl: 'https://feeds.example.com/mgp.rss',
      },
    });
  ...
  it('includes htmlUrl when the podcast has a siteUrl', async () => {
    await db.podcast.create({
      data: {
        title: 'Pod',
        feedUrl: 'https://feeds.example.com/pod.rss',
  ...
  it('orders podcasts alphabetically by title', async () => {
    await db.podcast.create({
      data: { title: 'Zebra Pod', feedUrl: 'https://z.example.com/feed.rss' },
    });
    await db.podcast.create({
      data: { title: 'Alpha Pod', feedUrl: 'https://a.example.com/feed.rss' },
    });
  ...
  it('escapes special XML characters in titles', async () => {
    await db.podcast.create({
      data: {
        title: 'Rock & Roll <Podcast>',
        feedUrl: 'https://feeds.example.com/rock.rss',
      },
    });
