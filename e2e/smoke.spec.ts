...
test.describe('Smoke', () => {
  test('app loads and shows the main layout', async ({ page, db: _ }) => {
    ...
    await expect(page.locator('[data-podhomme="episode-title"]')).toContainText('Nothing playing');
  });

  test('left pane shows All Podcasts and Favorites items', async ({ page, db: _ }) => {
    await page.goto('/');

    const list = page.getByRole('listbox', { name: 'Podcast library' });
    await expect(list.getByRole('option', { name: /All Podcasts/ })).toBeVisible();
